...
`subagent()` returns immediately. The sub-agent runs in its own terminal pane — a right split off the parent pi pane, so pane creation never steals keyboard focus. A live widget above the input tracks every running sub-agent, and when one finishes, its result is steered into the main session as a notification that triggers a new turn.
...
- [tmux](https://github.com/tmux/tmux) — `tmux new -A -s pi 'pi'`
- [WezTerm](https://wezfurlong.org/wezterm/) — just run pi inside WezTerm (no wrapper needed)

Optional: set `PI_SUBAGENT_MUX=tmux|wezterm` to force a specific backend.

Panes are kept evenly sized. With tmux, the extension re-applies an `even-horizontal` layout after every spawn and exit (debounced). The layout is a single constant, `SUBAGENT_TMUX_LAYOUT` in `pi-extension/subagents/tmux.ts` — change it to any named tmux layout (`main-vertical`, `tiled`, …). WezTerm handles pane layout automatically.
